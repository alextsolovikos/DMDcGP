...
class DMDcGPModel(torch.nn.Module):
    """ Gaussian process + DMDc reduced order dynamics """
    ...
    def train(self, training_dataset, training_iter=100, lr=0.1, path='data/gpdmdc_model.pth', 
        ...
        if self.nu > 0:
            U, Sig, VT = torch.linalg.svd(torch.vstack((H0, U0)), full_matrices=False)
        else:
            U, Sig, VT = torch.linalg.svd(H0, full_matrices=False)

        thres = 1.0e-10
        rtil = min((torch.sum(Sig > thres)).item(), self.nx)
        print('rtil = ', rtil, ' out of ', self.nx)
        Util = U[:,:rtil]
        Sigtil = torch.diag(Sig)[:rtil, :rtil]
        Vtil = VT.T[:,:rtil]

        # Transition matrix
        U_A = Util[:self.nx,:]
        self.A = H1 @ Vtil @ torch.inverse(Sigtil) @ U_A.conj().T

        # Control matrix (if it has inputs)
        if self.nu > 0:
            U_B = Util[self.nx:self.nx+self.nu,:]
            self.B = H1 @ Vtil @ torch.inverse(Sigtil) @ U_B.conj().T
